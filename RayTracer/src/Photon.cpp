...
{
    float low = 1.0f/this->Direction().X*(box.XMin - this->position_.X);
    float high = 1.0f/this->Direction().X*(box.XMax - this->position_.X);
    float tNear = std::min(low, high);
    float tFar = std::max(low, high);
