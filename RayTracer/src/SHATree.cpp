...
SHATree::SHATree(float emptySpaceCost, float maxNodeCost, unsigned int maxDepth): primaryBox_(0, 0, 0, 0, 0, 0),
                                            emptySpaceCost_(emptySpaceCost), maxNodeCost_(maxNodeCost),
                                            rootX_(nullptr), rootY_(nullptr), rootZ_(nullptr), maxDepth_(maxDepth)
{}

void SHATree::Initialize(std::vector<ISceneObject*>& objects)
{
...
SHATree::SHANode*SHATree::DivideAndBuild(SHATree::Axis axis, std::vector<std::pair<Box, ISceneObject*>>::const_iterator maxLeft,
...
float SHATree::GetBoxValueMinFromAxis(const Box& box, SHATree::Axis axis) const
{
    switch(axis)
    {
        case Axis::A_X:
...
CollisionData*SHATree::CollideNode(SHATree::SHANode* xNode, SHATree::SHANode* yNode, SHATree::SHANode* zNode,
